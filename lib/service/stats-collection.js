...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'tad-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('tad').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('tad')[service]);
